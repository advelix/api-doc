...
/**
 * Generates the RPC HTML document by delegating to the same Smart-doc builder the official
 * smart-doc-maven-plugin {@code rpc-html} goal uses, so the artifact is exactly what Smart-doc
 * produces for the same configuration.
 *
 * <p>Artifacts are written to the output directory: the RPC HTML document inside the output directory.
 *
 ...
 *
 * <pre>
 * mvn api-doc:rpc-html -Dsource=./src/main/java -Doutput=./docs
 * </pre>
 */
@Mojo(name = "rpc-html",
        ...
        threadSafe = false)
public class GenerateRpcHtmlMojo extends AbstractApiDocMojo {
    ...
    protected void generate(ApiConfig config, File outputDirectory, List<File> sourceRoots)
        ...
        SmartDocBuilders.buildRpcHtml(config, SmartDocBuilders.newJavaProjectBuilder(config, sourceRoots));
    }
}
