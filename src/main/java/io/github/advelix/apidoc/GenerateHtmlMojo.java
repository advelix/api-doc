...
/**
 * Generates the HTML document by delegating to the same Smart-doc builder the official
 * smart-doc-maven-plugin {@code html} goal uses, so the artifact is exactly what Smart-doc
 * produces for the same configuration.
 *
 * <p>Artifacts are written to the output directory: the api/ site plus AllInOne.html inside the output directory.
 *
...
@Mojo(name = "html",
        ...
        threadSafe = false)
public class GenerateHtmlMojo extends AbstractApiDocMojo {
    ...
    protected void generate(ApiConfig config, File outputDirectory, List<File> sourceRoots)
        ...
        SmartDocBuilders.buildHtml(config, SmartDocBuilders.newJavaProjectBuilder(config, sourceRoots));
    }
}
