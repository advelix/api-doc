...
/**
 * Generates the Markdown document by delegating to the same Smart-doc builder the official
 * smart-doc-maven-plugin {@code markdown} goal uses, so the artifact is exactly what Smart-doc
 * produces for the same configuration.
 *
 * <p>Artifacts are written to the output directory: per-group .md files plus AllInOne.md inside the output directory.
 *
...
@Mojo(name = "markdown",
        ...
        threadSafe = false)
public class GenerateMarkdownMojo extends AbstractApiDocMojo {
    ...
    protected void generate(ApiConfig config, File outputDirectory, List<File> sourceRoots)
        ...
        SmartDocBuilders.buildMarkdown(config, SmartDocBuilders.newJavaProjectBuilder(config, sourceRoots));
    }
}
