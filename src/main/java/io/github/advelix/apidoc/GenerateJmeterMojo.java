...
/**
 * Generates the JMeter test plan by delegating to the same Smart-doc builder the official
 * smart-doc-maven-plugin {@code jmeter} goal uses, so the artifact is exactly what Smart-doc
 * produces for the same configuration.
 *
 * <p>Artifacts are written to the output directory: the JMeter .jmx test plan inside the output directory.
 *
 ...
 *
 * <pre>
 * mvn api-doc:jmeter -Dsource=./src/main/java -Doutput=./docs
 * </pre>
 */
@Mojo(name = "jmeter",
        ...
        threadSafe = false)
public class GenerateJmeterMojo extends AbstractApiDocMojo {
    ...
    protected void generate(ApiConfig config, File outputDirectory, List<File> sourceRoots)
        ...
        SmartDocBuilders.buildJmeter(config, SmartDocBuilders.newJavaProjectBuilder(config, sourceRoots));
    }
}
