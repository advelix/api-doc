...
/**
 * Generates the Postman collection by delegating to the same Smart-doc builder the official
 * smart-doc-maven-plugin {@code postman} goal uses, so the artifact is exactly what Smart-doc
 * produces for the same configuration.
 *
 * <p>Artifacts are written to the output directory: the Postman collection JSON file inside the output directory.
 *
 ...
 *
 * <pre>
 * mvn api-doc:postman -Dsource=./src/main/java -Doutput=./docs
 * </pre>
 */
@Mojo(name = "postman",
        ...
        threadSafe = false)
public class GeneratePostmanMojo extends AbstractApiDocMojo {
    ...
    protected void generate(ApiConfig config, File outputDirectory, List<File> sourceRoots)
        ...
        SmartDocBuilders.buildPostman(config, SmartDocBuilders.newJavaProjectBuilder(config, sourceRoots));
    }
}
