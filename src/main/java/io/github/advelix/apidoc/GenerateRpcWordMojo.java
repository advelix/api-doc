...
/**
 * Generates the RPC Word document by delegating to the same Smart-doc builder the official
 * smart-doc-maven-plugin {@code rpc-word} goal uses, so the artifact is exactly what Smart-doc
 * produces for the same configuration.
 *
 * <p>Artifacts are written to the output directory: the RPC Word (docx) document inside the output directory.
 *
 ...
 *
 * <pre>
 * mvn api-doc:rpc-word -Dsource=./src/main/java -Doutput=./docs
 * </pre>
 */
@Mojo(name = "rpc-word",
        ...
        threadSafe = false)
public class GenerateRpcWordMojo extends AbstractApiDocMojo {
    ...
    protected void generate(ApiConfig config, File outputDirectory, List<File> sourceRoots)
        ...
        SmartDocBuilders.buildRpcWord(config, SmartDocBuilders.newJavaProjectBuilder(config, sourceRoots));
    }
}
