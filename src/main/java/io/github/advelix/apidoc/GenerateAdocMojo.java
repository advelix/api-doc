...
/**
 * Generates the AsciiDoc document by delegating to the same Smart-doc builder the official
 * smart-doc-maven-plugin {@code adoc} goal uses, so the artifact is exactly what Smart-doc
 * produces for the same configuration.
 *
 * <p>Artifacts are written to the output directory: per-group .adoc files plus AllInOne.adoc inside the output directory.
 *
 ...
 *
 * <pre>
 * mvn api-doc:adoc -Dsource=./src/main/java -Doutput=./docs
 * </pre>
 */
@Mojo(name = "adoc",
        ...
        threadSafe = false)
public class GenerateAdocMojo extends AbstractApiDocMojo {
    ...
    protected void generate(ApiConfig config, File outputDirectory, List<File> sourceRoots)
        ...
        SmartDocBuilders.buildAdoc(config, SmartDocBuilders.newJavaProjectBuilder(config, sourceRoots));
    }
}
