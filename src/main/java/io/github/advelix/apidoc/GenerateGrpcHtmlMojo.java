...
/**
 * Generates the gRPC HTML document by delegating to the same Smart-doc builder the official
 * smart-doc-maven-plugin {@code grpc-html} goal uses, so the artifact is exactly what Smart-doc
 * produces for the same configuration.
 *
 * <p>Artifacts are written to the output directory: the gRPC HTML document inside the output directory.
 *
 ...
 *
 * <pre>
 * mvn api-doc:grpc-html -Dsource=./src/main/java -Doutput=./docs
 * </pre>
 */
@Mojo(name = "grpc-html",
        ...
        threadSafe = false)
public class GenerateGrpcHtmlMojo extends AbstractApiDocMojo {
    ...
    protected void generate(ApiConfig config, File outputDirectory, List<File> sourceRoots)
        ...
        SmartDocBuilders.buildGrpcHtml(config, SmartDocBuilders.newJavaProjectBuilder(config, sourceRoots));
    }
}
