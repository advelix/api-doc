...
/**
 * Generates the Word (docx) document by delegating to the same Smart-doc builder the official
 * smart-doc-maven-plugin {@code word} goal uses, so the artifact is exactly what Smart-doc
 * produces for the same configuration.
 *
 * <p>Artifacts are written to the output directory: one index.docx per API group
 * (named after the group, for example {@code UserControllerindex.docx}) plus
 * {@code dict.docx} and {@code error.docx}.
 *
...
@Mojo(name = "word",
        ...
        threadSafe = false)
public class GenerateWordMojo extends AbstractApiDocMojo {
    ...
    protected void generate(ApiConfig config, File outputDirectory, List<File> sourceRoots)
        ...
        SmartDocBuilders.buildWord(config, SmartDocBuilders.newJavaProjectBuilder(config, sourceRoots));
    }
}
