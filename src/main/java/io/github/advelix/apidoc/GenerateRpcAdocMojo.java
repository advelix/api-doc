...
/**
 * Generates the RPC AsciiDoc document by delegating to the same Smart-doc builder the official
 * smart-doc-maven-plugin {@code rpc-adoc} goal uses, so the artifact is exactly what Smart-doc
 * produces for the same configuration.
 *
 * <p>Artifacts are written to the output directory: the RPC AsciiDoc document inside the output directory.
 *
 ...
 *
 * <pre>
 * mvn api-doc:rpc-adoc -Dsource=./src/main/java -Doutput=./docs
 * </pre>
 */
@Mojo(name = "rpc-adoc",
        ...
        threadSafe = false)
public class GenerateRpcAdocMojo extends AbstractApiDocMojo {
    ...
    protected void generate(ApiConfig config, File outputDirectory, List<File> sourceRoots)
        ...
        SmartDocBuilders.buildRpcAdoc(config, SmartDocBuilders.newJavaProjectBuilder(config, sourceRoots));
    }
}
